// FILE: src/components/TopNav.jsx (link -> <Link/>)
...
export default function TopNav({ brand = "deprowebs", href = "/", onLogin = () => {} }) {
  ...
  return (
    <motion.header className="fixed top-0 inset-x-0 z-[300] pointer-events-auto" style={{ backgroundColor: bg, boxShadow: shadow }}>
      ...
      >
        {/* Left: brand */}
        <Link to={href} className="flex items-center gap-2" aria-label={brand} onMouseEnter={closeAll}>
        ...
        <nav className="hidden lg:flex items-center gap-1 relative" onMouseLeave={closeAll}>
          {NAV.map((g) => (
            <TopItem key={g.key} group={g} open={openKey === g.key} setOpen={() => setOpenKey(g.key)} close={closeAll} />
          ))}
  ...
  return (
    <div className="relative group">
      ...
      <button
        ...
        onFocus={setOpen}
        className={`relative px-3 py-2 rounded-md text-sm font-semibold transition-colors ${open ? "text-white" : "text-white/80 hover:text-white"}`}
      >
        {group.label}
        <span
          className="pointer-events-none absolute left-2 right-2 -bottom-0.5 h-[2px] rounded-full transition-opacity"
          style={{ background: `linear-gradient(90deg, ${BLUE[700]}, ${BLUE[500]})`, opacity: open ? 1 : 0 }}
        />
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ border: "1px solid rgba(255,255,255,.10)", background: "linear-gradient(180deg, rgba(15,17,21,.92), rgba(5,12,34,.92))", boxShadow: `0 24px 60px ${BLUE[900]}80`, backdropFilter: "blur(10px)" }}
          >
            <div className="absolute -top-2 left-1/2 -translate-x-1/2 h-2 w-2 rotate-45" style={{ background: "rgba(15,17,21,.92)", borderLeft: "1px solid rgba(255,255,255,.10)", borderTop: "1px solid rgba(255,255,255,.10)" }} />
                  ...
                  <ul className="space-y-2">
                    {col.items.map((it) => (
                      <li key={it.to}>
                        <Link
                          to={it.to}
                          className="flex items-center gap-3 rounded-xl px-3 py-3 text-white/90 hover:text-white transition ring-1 ring-white/10 hover:ring-white/20"
                          style={{ background: "linear-gradient(180deg, rgba(255,255,255,.06), rgba(255,255,255,.035))" }}
                          onClick={close}
                        >
                          <span className="h-2.5 w-2.5 rounded-full" style={{ background: `linear-gradient(135deg, ${BLUE[700]}, ${BLUE[500]})` }} aria-hidden />
                          <span className="font-semibold">{it.label}</span>
                          <span className="ml-auto text-[10px] uppercase tracking-wide text-white/55">Explore</span>
                        </Link>
                      </li>
                    ))}
                  </ul>
