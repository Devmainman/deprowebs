...
export default function Hybridfooter() {
  return (
    <footer className="relative text-white overflow-hidden">
      {/* Top wave divider */}
      <svg className="absolute -top-[1px] left-0 right-0 w-full" viewBox="0 0 1440 80" preserveAspectRatio="none">
        ...
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          >
            ...
            >
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/10 border border-white/15 placeholder-white/50 focus:outline-none focus:border-blue-400"
                />
              </div>
              <button
                className="px-5 py-3 rounded-xl font-semibold btn-shine"
            ...
            <motion.div variants={fadeUp} className="flex items-center gap-3 mt-5 text-white/60 text-sm">
              <Shield size={16} /> We respect your privacy.
            </motion.div>
          </motion.div>
              ...
              <ul className="space-y-2">
                {col.links.map((l) => (
                  <motion.li key={l} variants={fadeUp}>
                    <a href="#" className="text-white/75 hover:text-white transition-colors">{l}</a>
                  </motion.li>
                ))}
              </ul>
            </motion.div>
        ...
        <div className="mt-12 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-white/10 pt-8">
          ...
          <div className="flex items-center gap-4">
            {[Facebook, Instagram, Youtube, Linkedin].map((Icon, i) => (
              <a key={i} href="#" className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:border-blue-400/50">
